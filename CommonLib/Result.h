...
template<typename T> using ResultConstRefT = typename ResultConstRef<T>::Type;
...
template<typename Derived, typename T, typename E> struct ResultAccessBase {
	...
	constexpr auto operator->() -> RemoveReferenceT<T> * { return &unwrap(); }
...
protected:
	...
	constexpr auto unwrap_err_const_impl(Derived const &self) const -> E const &
	{
		require_err(self.is_ok(), "Attempt to unwrap_err Ok Result");
		return self.get_err_unsafe();
	}
...
template<typename T, typename E> struct ResultStorage {
	...
	constexpr auto is_ok() const -> bool { return state == ResultState::Ok; }
...
template<typename Derived, typename T, typename E> struct ResultBase {
	...
	constexpr auto is_ok() const -> bool { return self().storage().is_ok(); }
	...
	template<typename Fn> constexpr auto unwrap_or_else(Fn &&fn) const & -> T
	{
		if (self().is_ok())
			return self().get_ok_unsafe();
...
protected:
	template<typename Fn> constexpr auto is_ok_and_impl(Fn &&fn) const -> bool
	{
		return self().is_ok() && fn(self().get_ok_unsafe());
	}

	template<typename Fn> constexpr auto is_err_and_impl(Fn &&fn) const -> bool
	{
...
protected:
	...
	{
		auto &self = static_cast<Derived &>(*this);
		require_err(self.storage().is_ok(), "Attempt to unwrap_err Ok Result");
