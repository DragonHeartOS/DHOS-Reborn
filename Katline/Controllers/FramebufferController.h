...
public:
    ...
    void draw_raw_character(char const ch, uint const y, uint const x, bool const inverted = false);
    void draw_character(char const ch, bool const inverted = false);

    void put_character(char const ch, bool const inverted = false);

    void put_string_safe(char const* string, size_t const size, bool const inverted = false);
    void put_string(char const* string, bool const inverted = false);

    void scroll_down(uint const lines = 1);

    void put_logo(u8 const* data, uint const width, uint const height, uint const x, uint const y);
